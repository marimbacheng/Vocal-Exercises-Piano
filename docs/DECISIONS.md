...
- **換 key 和弦補滿一整拍**:速度是二分音符 BPM,一整拍 = 2 個四分拍,故「單和弦間隔」的新調提示和弦 `beats` 由 1 改 2(原本只佔半拍、聽感沒填滿)。
- **`triplet` 旗標更名 `singleChordGap`**:語意是「換 key 間隔只放新調和弦(無當前調和弦)」,與音型是否三連音脫鉤。設於八度系列、五度跳、八度大跳。
- **ext-13 結尾回主音 + 標準兩段和弦間隔**:第 4 拍三連音第 3 顆由 `2` 改回主音 `1`(唱到主音收束);**不設 `singleChordGap`**,沿用標準間隔(當前調 1-3-5 + 新調 1-3-5),取得「琶音 ×1」那樣的結尾主和弦聽感。落選:以主音單音當「當前調和弦」——使用者要的是真三和弦聽感,故用標準 gapCurrent(buildTriad 齊奏)。

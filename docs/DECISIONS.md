...
- **半速三連音換 key 間隔:和弦各佔一整拍**(`tripletGap` 旗標,取代 `singleChordGap`):半速三連音的脈動 = 一個三連音組 = 2 個四分拍(一整拍)。為維持在 grid 上(「節奏目前是對的」),換 key 的提示和弦每個佔 2 拍,而非標準間隔的 gapBeats/2(半拍、離格)。
  - `'both'`(八度系列、ext-13):當前調 1-3-5(接過去)+ 新調 1-3-5,各 2 拍。
  - `'both'` 現用於全部半速三連音音型(八度系列、ext-13、五度跳、八度大跳)。
  - `'nextOnly'`(只放新調和弦 2 拍)仍為支援選項、有測試涵蓋,但目前無內建音型使用。
  - 一般音型未設 → 標準間隔不變。`plan.ts` 依旗標分支;`player` gap 狀態由 gapCurrent 或 gapNext 觸發。
- **ext-13 結尾定案**:骨架 `1-3-5 / 8-10-12 / 11-9-7 / 5-4-2 / 1-0-1`,末組 `1-0-1`(degree 0 = root−1,下鄰音回主音)唱回主音收束;之後接當前調 + 新調兩個一整拍和弦(`tripletGap:'both'`),結尾和弦聽感類似「琶音 ×1」。

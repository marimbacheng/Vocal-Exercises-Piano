...
describe('buildSessionTimeline — 三連音換 key 間隔(tripletGap 旗標)', () => {
  const arp = parsePatternDsl('1:0.33333 3:0.33333 5:0.33333 1:0.66666 1:0.33333');

  describe("'nextOnly':只放新調和弦一整拍", () => {
    const tl = buildSessionTimeline({ ...baseParams, pattern: arp, topRoot: 62, tripletGap: 'nextOnly' });
    const triads = tl.events.filter((e): e is TriadEvent => e.kind === 'triad');

    it('無 gapCurrent,gapNext 數 = run-1', () => {
      expect(triads.filter((t) => t.role === 'gapCurrent')).toHaveLength(0);
      expect(triads.filter((t) => t.role === 'gapNext')).toHaveLength(tl.runCount - 1);
    });

    it('新調 triad 長一整拍(2 拍)、和弦為下一 key 的 1-3-5', () => {
